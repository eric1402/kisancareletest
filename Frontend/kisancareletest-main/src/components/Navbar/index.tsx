...
export function Navbar() {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-6 sm:pt-5">
      ...
      >
        <div
          className={cn("pointer-events-none absolute inset-0 rounded-[18px] transition-opacity duration-300", isLight ? "bg-gradient-to-b from-white/60 to-white/20 opacity-100" : "bg-gradient-to-b from-white/[0.08] to-transparent opacity-60")}
          aria-hidden
        />
        <div className={cn("pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r transition-opacity", isLight ? "from-transparent via-black/10 to-transparent" : "from-transparent via-white/20 to-transparent")} aria-hidden />
        ...
        <div className="relative hidden items-center gap-1 lg:flex">
          {navLinks.map((link, idx) => (
            ...
            >
              {link.label}
              {idx === 0 && (
                <span className="absolute inset-x-3 -bottom-1 h-[2px] rounded-full bg-[#2a8a3a] shadow-[0_0_8px_rgba(42,138,58,0.35)]" aria-hidden />
              )}
            </a>
          ))}
        </div>
