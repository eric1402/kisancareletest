...
function CropCalendarPage() {
  ...
  return (
    <div className="flex h-dvh max-h-dvh w-full overflow-hidden bg-[#F9FAFB] antialiased">
      {/* ── Sidebar ── */}
      <Sidebar mobileOpen={mobileOpen} onCloseMobile={() => setMobileOpen(false)} />

      {/* ── Right shell ── */}
      <div className="flex flex-1 flex-col min-h-0 h-dvh max-h-dvh overflow-hidden">
        <div className="shrink-0">
          <DashboardHeader onToggleMobileSidebar={() => setMobileOpen(p => !p)} />
        </div>

        {/* ── Scrollable body ── */}
        <main className="flex-1 min-h-0 overflow-hidden flex flex-col px-5 py-3"
          ...
          {isLoading ? (
            ...
            <>
          {/* ── PAGE HEADER ── */}
          <div className="flex items-start justify-between mb-2.5 gap-4 shrink-0 animate-dash-1">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <div className="w-7 h-7 rounded-lg bg-green-100 flex items-center justify-center">
                  <Calendar className="w-[15px] h-[15px] text-kc-green" />
                </div>
                <h1 className="text-[21px] font-bold text-gray-900 tracking-tight leading-none">
                  Crop Calendar
                </h1>
              </div>
              <p className="text-[13px] text-gray-500 leading-relaxed ml-0.5">
                Plan your farming activities with the right timing. Get crop-wise guidance for sowing, growing and harvesting.
              </p>
            </div>
          ...
          {/* ── 3-COLUMN GRID ── */}
          <div className="flex-1 min-h-0 grid gap-3 animate-dash-3" style={{ gridTemplateColumns:"210px 1fr 252px" }}>

            {/* ════════ LEFT: Select Crop ════════ */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-xs p-3.5 flex flex-col gap-2 h-full overflow-hidden">
