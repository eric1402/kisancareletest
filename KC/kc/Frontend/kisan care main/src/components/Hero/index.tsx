...
export function Hero() {
  ...
  const ref = useGsapScope<HTMLDivElement>((ctx) => {
    ...
    gsap.fromTo("[data-hero-bg]", { scale: 1.08 }, { scale: 1, duration: 1.8, ease: "power3.out" });

    const tl = gsap.timeline({ defaults: { ease: "power3.out" } });
    tl.from(
      "[data-reveal]",
      { y: 22, opacity: 0, duration: 0.7, stagger: 0.09, clearProps: "transform" },
      0.15,
    )
      .from(
        "[data-card]",
        { y: 18, opacity: 0, duration: 0.6, stagger: 0.07, ease: "power2.out" },
        0.55,
      )
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(95% 55% at 50% 115%, rgba(3,27,18,0.96) 0%, transparent 62%)",
        }}
      />
      ...
      <div className="relative mx-auto flex w-full max-w-[1280px] flex-col gap-8 px-4 sm:px-6 lg:grid lg:grid-cols-[1.06fr_485px] lg:items-center lg:gap-6 lg:px-6 xl:px-2">
        ...
        <div data-card className="relative w-full will-change-transform lg:pt-8">
          ...
          >
            ...
            <div
              data-soil
              className="mt-3 flex items-center justify-between rounded-[16px] border border-white/[0.07] bg-[rgba(0,0,0,0.28)] px-3.5 py-3.5 backdrop-blur-md will-change-transform sm:px-4"
            >
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-medium tracking-[0.04em] text-white/55">
                  Soil Moisture • NPK
                </p>
                <p className="mt-1 text-[13.5px] font-semibold text-white sm:text-[14px]">
                  62% • N Low • P Medium
                </p>
                <div className="mt-2.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                  <div
                    ...
                    style={{
                      width: "62%",
                      background: "linear-gradient(90deg, #9BEA3C 0%, #35A853 100%)",
                    }}
                  />
                </div>
              </div>
